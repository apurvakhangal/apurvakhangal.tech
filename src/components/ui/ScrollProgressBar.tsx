...
type ProgressMode = 'floating' | 'docked' | 'fixed';
...
export function ScrollProgressBar({
  ...
  const containerClass =
    containerClassName ||
    (isFloating
      ? 'fixed top-0 left-0 w-full px-4 md:px-24 z-40 pointer-events-none'
      : isFixed
        ? 'fixed top-[80px] left-1/2 -translate-x-1/2 w-[90%] max-w-[1200px] z-40 pointer-events-none h-[40px] flex items-center'
        : 'relative w-full px-4 md:px-24 z-40 pointer-events-none');

  return (
    <motion.div
      style={isFloating ? { y } : undefined}
    ...
    >
      <div className="relative w-full h-[40px] flex items-center">
        ...
        <motion.div
          style={{ opacity: isFloating ? initialOpacity : 0 }}
        ...
        >
          <div className="flex justify-between w-full opacity-30 select-none">
            {sections.map((_, idx) => (
              <span key={`cross-${idx}`} className="text-white text-sm font-mono leading-none">
                +
              </span>
            ))}
          </div>
        ...
        <motion.div
          style={{ opacity: isFloating ? progressOpacity : 1 }}
        ...
        >
          <div className="absolute left-0 right-0 flex justify-between w-full items-center h-full">
            {Array.from({ length: 5 }).map((_, idx) => (
              <div key={idx} className="flex flex-col items-center justify-center bg-[#111625] px-3">
                <span className="text-[12px] font-mono text-white/50 tracking-widest">
                  {`0${idx + 1}`}
                </span>
              </div>
            ))}
